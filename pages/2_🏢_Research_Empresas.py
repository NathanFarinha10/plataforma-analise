# pages/2_🏢_Research_Empresas.py (Arquitetura Final com yfinance DCF)
...
def get_dcf_data_from_yf(ticker_symbol):
    """Busca e calcula os dados para o DCF usando exclusivamente yfinance."""
    try:
        ticker = yf.Ticker(ticker_symbol)
        ...
        # FCF = Fluxo de Caixa Operacional + Despesas de Capital (CapEx é negativo)
        ...
        # Dívida Líquida = Passivos Totais - Ativos Totais + Caixa (rearranjo da equação contábil)
        # Uma abordagem mais direta é Dívida Total - Caixa
        total_liab = balance_sheet.loc['Total Liab'].iloc[0]
        total_cash = balance_sheet.loc['Total Cash'].iloc[0]
        net_debt = total_liab - total_cash # Simplificação, idealmente seria Long Term Debt + Short Term Debt - Cash
