...
with tab_br:
    ...
    with subtab_br_activity:
        ...
        data = fetch_bcb_series({'data': 24369}, start_date); plot_indicator_with_analysis(data['data'], "IBC-Br (Prévia do PIB)", "Índice de Atividade Econômica do BCB.", "Índice")
        st.divider()
        data = fetch_bcb_series({'data': 21859}, start_date); plot_indicator_with_analysis(data['data'].pct_change(12).dropna()*100, "Produção Industrial", "Mede a produção física da indústria.", "Var. Anual %")
        st.divider()
        data = fetch_bcb_series({'data': 1473}, start_date); plot_indicator_with_analysis(data['data'].pct_change(12).dropna()*100, "Vendas no Varejo", "Mede o volume de vendas do comércio.", "Var. Anual %")
        st.divider()
        data = fetch_bcb_series({'data': 24424}, start_date); plot_indicator_with_analysis(data['data'].pct_change(12).dropna()*100, "Volume de Serviços", "Mede a receita real do setor de serviços.", "Var. Anual %")
        st.divider()
        data = fetch_bcb_series({'data': 4393}, start_date); plot_indicator_with_analysis(data['data'], "Índice de Confiança do Consumidor", "Mede o otimismo dos consumidores.", "Índice")

    with subtab_br_jobs:
        st.subheader("Indicadores do Mercado de Trabalho Brasileiro")
        st.divider()
        data = fetch_bcb_series({'data': 24369}, start_date); plot_indicator_with_analysis(data['data'], "Taxa de Desemprego (PNADC)", "Porcentagem da força de trabalho desocupada.", "%")
        st.divider()
        data = fetch_bcb_series({'data': 28795}, start_date); plot_indicator_with_analysis(data['data'], "Renda Média Real (Trabalhador com Carteira)", "Variação anual do rendimento médio real do trabalhador com carteira assinada.", "Var. Anual %")
        st.divider()
        data = fetch_bcb_series({'data': 28793}, start_date); plot_indicator_with_analysis(data['data'], "Renda Média Real (Total Setor Privado)", "Variação anual do rendimento médio real de todos os trabalhadores do setor privado.", "Var. Anual %")

    with subtab_br_inflation:
        st.subheader("Indicadores de Inflação e Preços")
        st.divider()
        c1, c2 = st.columns(2)
        with c1: data = fetch_bcb_series({'data': 433}, start_date); plot_indicator_with_analysis(data['data'], "IPCA (Variação Mensal)", "Mede a inflação oficial do país.", unit="%")
        with c2: data = fetch_bcb_series({'data': 11427}, start_date); plot_indicator_with_analysis(data['data'], "Média dos Núcleos de Inflação", "Mede a tendência de fundo da inflação.", unit="%")
        st.divider()
        c3, c4 = st.columns(2)
        with c3: data = fetch_bcb_series({'data': 4449}, start_date); plot_indicator_with_analysis(data['data'], "IPCA - Bens Industrializados", "Inflação de produtos industriais.", unit="%")
        with c4: data = fetch_bcb_series({'data': 4448}, start_date); plot_indicator_with_analysis(data['data'], "IPCA - Serviços", "Inflação do setor de serviços.", unit="%")
        st.divider()
        data = fetch_bcb_series({'data': 189}, start_date); plot_indicator_with_analysis(data['data'], "IGP-M (Variação Mensal)", "A 'inflação do aluguel'.", unit="%")
